...
public class ProfileService : IProfileService
{
    ...
    public async Task<ProfileDto?> GetProfileAsync(Guid userId)
    {
        var user = await _unitOfWork.Users.FirstOrDefaultAsync(
            u => u.Id == userId,
            includeProperties: new[] { nameof(User.Role) }); // JOIN lấy luôn vai trò
    ...
    // Lấy lịch sử điểm, mới nhất lên đầu (sắp xếp ngay tại SQL)
    public async Task<List<PointHistoryDto>> GetPointHistoryAsync(Guid userId)
    {
        var histories = await _unitOfWork.RewardPointHistories.GetAllAsync(
            h => h.UserId == userId,
            orderBy: q => q.OrderByDescending(h => h.CreatedAt));

        return histories.Select(h => _mapper.Map<PointHistoryDto>(h)).ToList();
    }
}
